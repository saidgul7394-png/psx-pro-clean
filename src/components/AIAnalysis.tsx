...
export default function AIAnalysis({ stocks, selectedSymbol, lang, onSelectStock }: AIAnalysisProps) {
  ...
  const fetchAnalysis = async (symbolToFetch: string) => {
    ...
    try {
      ...
      if (!contentType || !contentType.includes("application/json")) {
        throw new Error("Received non-JSON response from analysis API");
      }
      const data: AIAnalysisResult = await res.json();
      setAnalysis(data);
    } catch (e: any) {
      console.warn("AI Analysis Fetch Warning:", e.message || e);
  ...
  const handleSendChat = async (textToSend: string) => {
    ...
    try {
      ...
      const contentType = res.headers.get("content-type");
      if (!contentType || !contentType.includes("application/json")) {
        throw new Error("Received non-JSON response from chat API");
      }
      ...
      console.warn("AI Chat Fetch Warning:", e.message || e);
